...
struct _interpreter
{
  ...
  /* For now, _interpreter is implemented as a singleton since its currently
     ...
     without patching the python source code or starting a separate process for
     each.
...
// must be called before the first regular call to matplotlib to have any
// effect
...
{ const static NPY_TYPES type = NPY_NOTYPE; };  // Default
...
template <typename Numeric>
PyObject * get_listlist(const std::vector<std::vector<Numeric>> & listlist) {
  PyObject * list = PyList_New(listlist.size());
  for (std::size_t i = 0; i < listlist.size(); ++i) {
    PyList_SetItem(list, i, get_array(listlist[i]));
  }
  return list;
}

#else  // fallback if we don't have numpy: copy every element of vector
...
inline bool subplots_adjust(
  ...
  for (const auto & keyword : keywords) {
    PyDict_SetItemString(
      kwargs, keyword.first.c_str(), PyFloat_FromDouble(keyword.second));
  }
...
// Is there a reason not to call this automatically for every plot?
